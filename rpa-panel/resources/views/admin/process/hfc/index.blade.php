...
function setDT1(){
        ...
        processing: true,         
        serverSide: true,
        ajax: "{{ route('admin.process.hfc.showhfc',1) }}",        
        columns: [
 			{ data: "TRNREFNO" },
            { data: "APPLNO" },
...
function setDT2(){
    ...
    if(date == undefined){
        ...
        ajax: {
            url:  "{{ route('admin.process.hfc.showhfc1',1) }}"
        },
...
function setDT3(){ 
    ...
    $("#dt-3").DataTable( {
        ...
        order : [[ 1, "desc" ]],
        search: { "caseInsensitive": false },
        ajax: "{{ route('admin.process.hfc.showhfc2',1) }}",
...
function setDT4(){ 
 $('#upload_csv').on("submit", function(e){
    $.ajaxSetup({
        ...
        $.ajax({
            //url:  "{{ route('admin.process.hfc.uploads',1) }}"
            url: "{{url('admin/process/hfc/uploads')}}", // point to server-side PHP script
            data: form_data,
            type: 'POST',
