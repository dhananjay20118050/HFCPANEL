...
class BOT_model extends CI_Model {
    ...
    function fincoreProcess($driver, $row){
    ...
    public function defaultpan($driver,$row){
        ...
        if(count($results) > 0){
            ...
            $sql = "UPDATE hfccustdata SET cifid_1 = '".$custId."' WHERE TRNREFNO = '".$row['TRNREFNO']."'";
            $qparent = $this->db->query($sql);
    ...
    public function addNewCustId($driver, $row, $cfid){
      ...
      /* $driver->wait(5)->until(WebDriverExpectedCondition::alertIsPresent());
        ...
        //if merge upper code then remove this line
        /*$driver->switchTo()->defaultContent();
        ...
        //echo "COUNT WINDOWS ---". count($handle);exit;
        //$driver->switchTo()->defaultContent();
        ...
        // $driver->findElement(WebDriverBy::id("tab_tpagePhone"))->click();
        ...
        //$phonehandle = $driver->getWindowHandle();
       ...
       // sleep(2);
        ...
        //echo count($driver->getWindowHandles());exit;
        //$driver->switchTo()->window($handle[0]); 
   //echo $driver->getPageSource();exit;     
       // $driver->switchTo()->defaultContent();
       ...
       // $driver->findElement(WebDriverBy::name("Save"))->click();
        //$driver->findElement(WebDriverBy::name("Close"))->click();
       ...
       /* $driver->findElement(WebDriverBy::id("tab_tpageCont5"))->click();
        ...
        //CURRENCY TAB
        $driver->findElement(WebDriverBy::name("tab_tpageCont6"))->click();
    ...
    public function saveCustID($driver, $appFormNo,$cfid){
        $selector = WebDriverBy::name("hidCustomerID");
        $hidCustomerID = $driver->findElement($selector)->getAttribute('value');
        $sql = "UPDATE hfccustdata SET $cfid = $hidCustomerID where TRNREFNO = '$appFormNo'";
        $qparent = $this->db->query($sql);
    }
