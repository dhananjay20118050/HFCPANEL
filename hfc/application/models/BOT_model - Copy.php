...
class BOT_model extends CI_Model {
    ...
    public function fincoreProcess($driver,$userId,$row){
        ...
        $driver->findElement(WebDriverBy::id("gotomenu"))->click();
            ...
            try {
                $this->defaultpan($driver,$userId,$row);
            } catch (Exception $e) {
    ...
    public function defaultpan($driver,$userId,$row){
       ...
       // $driver->switchTo()->defaultContent();
        ...
        if(!empty($row['JH1PAN'])){
            $cifid = 'cifid_2';
            try {
                $this->checkpan($driver,$row['JH1PAN'],$row['TRNREFNO'],$cifid);
            } catch (Exception $e) {
        ...
        if(!empty($row['JH2PAN'])){
            ...
            try {
                $this->checkpan($driver,$row['JH2PAN'],$row['TRNREFNO'],$cifid);
            } catch (Exception $e) {
        ...
        if(empty($row['cifid_2']) && !empty($row['JH1PAN'])){
            ...
            try {
                $this->addNewCustId($driver, $row,'cifid_2');
            } catch (Exception $e) {
        ...
        if(empty($row['cifid_3']) && !empty($row['JH2PAN'])){
            ...
            try {
                $this->addNewCustId($driver, $row,'cifid_3');
            } catch (Exception $e) {
    ...
    public function checkpan($driver, $pan,$trnrefno,$cifid){
        ...
        if(count($results) > 0){
            ...
            $sql = "UPDATE hfccustdata SET $cifid = '".$custId."' WHERE TRNREFNO = '".$trnrefno."'";
            $qparent = $this->db->query($sql);
