...
class BOT_model extends CI_Model {
    ...
    /*public function finalfun($userId, $trnrefno){

        $sql = "SELECT a.*,b.is_pan_checked FROM hfccustdata a LEFT JOIN bot_aps_tracking b on a.TRNREFNO = b.TRNREFNO WHERE b.status in ('Y','EE','PP') AND a.TRNREFNO = '$trnrefno' ORDER BY id";
        ...
        if(count($rows) > 0){
            ...
            $sql = "UPDATE bot_aps_tracking SET status = 'PP', ip_address = '".$localIP."', start_time = '".date("Y-m-d H:i:s")."' WHERE TRNREFNO = '".$row['TRNREFNO']."'";
            $qparent = $this->db->query($sql);
      
          try {
                $this->loginCust($driver, $row,$userId);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "Finacle Login For Accont Creation", $driver,'finalexception');
            }
            ...
            $sql = "UPDATE bot_aps_tracking SET status = 'F', end_time = '".date("Y-m-d H:i:s")."' WHERE TRNREFNO = '".$row['TRNREFNO']."'";
            $qparent = $this->db->query($sql);
      }
        $driver->quit();
    ...
    public function startCRMProcess($driver,$userId,$row){
        ...
        if(!empty($row['cifid_1']) && $row['is_existing_cust_1'] == 1){
            $j++;
            try {
        ...
        if(!empty($row['cifid_2']) && $row['is_existing_cust_2'] == 1){
            $j++;
           try {
        ...
        if(!empty($row['cifid_3']) && $row['is_existing_cust_3'] == 1){
            $j++;
              try {
    ...
    public function addNewCustId($driver, $row, $cfid, $dob, $name, $pan,$num,$userid){
       if($num == '1'){
        ...
        try {
            ...
            $this->addException($e, $row['TRNREFNO'], $userid, "ADD NEW CUST ID", $driver);
        }
    ...
    public function editgeneralentry($driver, $row, $cfid, $dob, $name, $pan){
        ...
        if(!empty($row['ADD2']) && !empty($row['ADD3'])){
           ...
           // $element->sendKeys($row['STATE']);
            ...
            //$element = $driver->findElement(WebDriverBy::name("AccountBO.Address.country"))->clear();
            //$element->sendKeys('IN');

            //$element = $driver->findElement(WebDriverBy::name("Cat_AccountBO.Address.country"))->clear();
            //$element->sendKeys('INDIA');
    ...
    public function accountcreation($driver,$row,$userId){
       ...
       // $driver->findElement(WebDriverBy::name("tdacop.cifId"))->sendKeys('661040053');
        ...
        $driver->findElement(WebDriverBy::name("tdgen.acctOpenDate_ui"))->sendKeys('29/08/2019');
    ...
    public function addException($e, $trnrefno, $userId, $error_section, $driver,$checkexception){
        ...
        if($checkexception != 'finalexception'){
            $sql = "UPDATE bot_aps_tracking SET status = 'E', end_time = '".date("Y-m-d H:i:s")."' WHERE TRNREFNO = '".$trnrefno."'";
        }else{
            $sql = "UPDATE bot_aps_tracking SET status = 'EE', end_time = '".date("Y-m-d H:i:s")."' WHERE TRNREFNO = '".$trnrefno."'";
        }
        $qparent = $this->db->query($sql);
        try {
