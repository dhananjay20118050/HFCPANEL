...
class BOT_model extends CI_Model {
    ...
    public function start($userId, $trnrefno){
        ...
        if(count($rows) > 0){
            ...
            try {
                $this->fincoreProcess($driver,$userId,$row);
            } catch (Exception $e) {
    ...
    public function fincoreProcess($driver,$userId,$row){
        ...
        $driver->findElement(WebDriverBy::id("gotomenu"))->click();
            ...
            try {
                $this->defaultpansearch($driver,$userId,$row);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "CIFID Search", $driver);
            }
        
    }

    public function defaultpansearch($driver,$userId,$row){
        $driver->wait(60,1000)->until(
        ...
        if(count($results) > 0){
            ...
            $sql = "UPDATE hfccustdata SET cifid_1 = '".$custId."' WHERE TRNREFNO = '".$row['TRNREFNO']."' and is_existing_cust = '1' ";
            $qparent = $this->db->query($sql);
        ...
        if(!empty($row['JH1PAN'])){
            $cifid = 'cifid_2';
            $is_existing_cust = 'is_existing_cust1';
            try {
                $this->checkpan($driver,$row['JH1PAN'],$row['TRNREFNO'],$cifid,$is_existing_cust);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "CIFID2 Search", $driver);
            }
            
        }
        if(!empty($row['JH2PAN'])){
            $cifid = 'cifid_3';
            $is_existing_cust = 'is_existing_cust2';
            try {
                $this->checkpan($driver,$row['JH2PAN'],$row['TRNREFNO'],$cifid,$is_existing_cust);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "CIFID3 Search", $driver);
            }
        }
        if(empty($row['cifid_1'])){
            try {
                $this->addNewCustId($driver, $row,'cifid_1');
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "ADDNEW CFID", $driver);
            }
            
        }
        if((is_null($row['cifid_2']) || empty($row['cifid_2'])) && !empty($row['JH1PAN'])){
            try {
                $this->addJoinholder($driver, $row,'cifid_2',$row['JH1NAME'],$row['JH1PAN']);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "ADDNEW2 CFID", $driver);
            }
        ...
        if((is_null($row['cifid_3']) || empty($row['cifid_3'])) && !empty($row['JH2PAN'])){
            ...
            try {
                $this->addJoinholder($driver, $row,'cifid_3',$row['JH2NAME'],$row['JH2PAN']);
            } catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "ADDNEW3 CFID", $driver);
            }
           
        }
        if(!empty($row['cifid_1'])){
            try {
                $this->editcust($driver,$row);
            }
            catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "EDIT CFID1", $driver);
            }
        }
        if(!empty($row['cifid_2'])){
             try {
                $this->editcust($driver,$row);
            }
            catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "EDIT CFID2", $driver);
            }
        }
        if(!empty($row['cifid_3'])){
             try {
                $this->editcust($driver,$row);
            }
            catch (Exception $e) {
                $this->addException($e, $row['TRNREFNO'], $userId, "EDIT CFID3", $driver);
            }
        }
      
    }

    public function editcust($driver,$row){
        $driver->switchTo()->defaultContent();
        ...
        $frame = $driver->findElement(WebDriverBy::xpath(".//iframe[@src='../servlet/com.infy.cis.ui.cif.ScreenPop_Unidentified_CIF?CalledFrom=360Deg&ScreenName=RetEdit&Type=Edit Entity']"));
        $driver->switchTo()->frame($frame);
        ...
        $element = $driver->findElement(WebDriverBy::name("FilterParam1"));
        $element->sendKeys($row['cifid_1']);

        $driver->findElement(WebDriverBy::name("submitBut"))->click();

    }

    public function checkpan($driver, $pan,$trnrefno,$cifid){
        ...
        if(count($results) > 0){
            ...
            $sql = "UPDATE hfccustdata SET $cifid = '".$custId."' WHERE TRNREFNO = '".$trnrefno."' and $is_existing_cust ='1'";
            $qparent = $this->db->query($sql);
    ...
    public function addNewCustId($driver, $row, $cfid){
...
//Address Details
       ...
       //start mail
        ...
        $driver->manage()->timeouts()->implicitlyWait = 10;
        /*$driver->wait(60,1000)->until(
            WebDriverExpectedCondition::visibilityOfElementLocated(WebDriverBy::name("frm2"))
        );*/
    ...
    public function addJoinholder($driver, $row, $cfid,$name,$pan){
...
//Address Details
    ...
    public function saveCustID($custid, $trnrefno,$cfid){
        ...
        $sql = "UPDATE bot_aps_tracking SET status = 'Y', end_time = '".date("Y-m-d H:i:s")."' WHERE TRNREFNO = '".$trnrefno."'";
        $qparent = $this->db->query($sql);
